...
let players = [], details = {}, tab = "all", sortMode = "rank", masterQuery = "";
let consensus = {};
...
function render() {
  const key = masterQuery.trim().toLowerCase();
  const shown = sortPlayers(players.filter(p => (tab === "all" || p.style === tab) && (!key || `${p.alias} ${p.player_id}`.toLowerCase().includes(key))));
  const searchStatus = document.querySelector("#master-search-status");
  if (searchStatus) searchStatus.textContent = key ? `找到 ${shown.length} 位符合的主力；點擊卡片查看檔案` : "直接篩選排行榜，點擊卡片查看個別主力";
...
document.querySelector("#master-search")?.addEventListener("input", event => { masterQuery = event.target.value; render(); });
