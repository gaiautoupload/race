...
export default function Home() {
  return <>
    <main className="race">
      ...
      <section className="search-hub"><form className="search-panel stock-panel" action="/stock/"><label htmlFor="stock-search">個股情報搜尋</label><div><input id="stock-search" name="q" type="search" placeholder="代號或名稱，例如 6696、仁新" autoComplete="off" /><button type="submit">查詢個股 →</button></div><small>均價、金額階梯、前 20 大買賣主力與昨日差異</small></form><section className="search-panel master-panel"><label htmlFor="master-search">主力搜尋</label><input id="master-search" type="search" placeholder="輸入主力代號或名稱，例如 海鯨、W01" autoComplete="off" /><small id="master-search-status">直接篩選排行榜，點擊卡片查看個別主力</small></section></section>
