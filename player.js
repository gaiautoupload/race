...
const spotlight = (title, rows, kind) => `<section class="spotlight ${kind}"><div><p>${kind === "buy" ? "TOP NET BUY · 大量掃貨" : kind === "sell" ? "TOP NET SELL · 大量出貨" : "NEW POSITION · 新建倉"}</p><h2>${title}</h2></div>${rows.length ? `<div class="spot-items">${rows.slice(0, 3).map((x, i) => `<div><em>${i + 1}</em><b>${esc(x.stock_code)} ${esc(x.stock_name)}</b><span>${esc(x.amount_label || `約 ${Number(x.amount_wan || 0).toLocaleString()} 萬`)} · ${Number(x.lots).toLocaleString()} 張</span>${kind === "new" ? `<small>最早買入 ${esc(x.first_buy_date || "—")}</small>` : ""}</div>`).join("")}</div>` : `<div class="empty">目前沒有大額紀錄</div>`}</section>`;
